...
export function openDB() {
    ...
    _dbPromise = new Promise((resolve, reject) => {
        ...
        req.onupgradeneeded = (event) => {
            ...
            // v3 - renewals (insurance, rates, ESL etc. with renewal cadence)
            if (!db.objectStoreNames.contains(STORE_RENEWALS)) {
...
/* ======================= Renewals (insurance, rates, ESL) ======================= */

export async function getAllRenewals() {
    const store = await tx(STORE_RENEWALS, 'readonly');
    return reqToPromise(store.getAll());
}

export async function addRenewal(renewal) {
    const store = await tx(STORE_RENEWALS, 'readwrite');
    return reqToPromise(store.add(renewal));
}

export async function updateRenewal(id, newValues) {
    const store = await tx(STORE_RENEWALS, 'readwrite');
    const existing = await reqToPromise(store.get(id));
...
/* ======================= Bulk import (for full-backup restore) ======================= */
// Each "importAll<Store>" clears the existing store then re-adds the supplied records.
// Used by the export/import buttons in Settings to back up & restore every store.

export async function importAllBudgets(records) {
    const db = await openDB();
    return new Promise((resolve, reject) => {
        const t = db.transaction([STORE_BUDGETS], 'readwrite');
        const store = t.objectStore(STORE_BUDGETS);
        store.clear().onsuccess = () => {
            // Budgets are keyed by 'category' (a meaningful string), so `put` is idempotent.
...
export async function importAllFavorites(records) {
    const db = await openDB();
    return new Promise((resolve, reject) => {
        const t = db.transaction([STORE_FAVORITES], 'readwrite');
        const store = t.objectStore(STORE_FAVORITES);
        store.clear().onsuccess = () => {
            records.forEach(rec => {
                const copy = { ...rec };
                delete copy.id; // re-issue auto-increment IDs after clear
                store.add(copy);
            });
        };
        t.oncomplete = () => resolve();
...
export async function importAllRenewals(records) {
    const db = await openDB();
    return new Promise((resolve, reject) => {
        const t = db.transaction([STORE_RENEWALS], 'readwrite');
        const store = t.objectStore(STORE_RENEWALS);
        store.clear().onsuccess = () => {
            records.forEach(rec => {
                const copy = { ...rec };
                delete copy.id;
